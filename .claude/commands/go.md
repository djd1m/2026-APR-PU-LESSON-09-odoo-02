...
## Step 6: Phase Artifact Verification (MANDATORY)

**KNOWN BUG (2026-05-21):** In autonomous mode the LLM skips Phases 1, 2, 4
and only does Phase 3 (IMPLEMENT). This verification step catches that.

After the pipeline returns, verify ALL required artifacts exist:

```
IF /feature was selected:
  CHECK docs/features/<feature>/01_specification.md   — Phase 1
  CHECK docs/features/<feature>/02_pseudocode.md      — Phase 1
  CHECK docs/features/<feature>/03_architecture.md    — Phase 1
  CHECK docs/features/<feature>/04_refinement.md      — Phase 1
  CHECK docs/features/<feature>/05_completion.md      — Phase 1
  CHECK docs/features/<feature>/validation-report.md  — Phase 2
  CHECK docs/features/<feature>/review-report.md      — Phase 4

IF /plan was selected:
  CHECK docs/plans/<feature>.md

IF ANY artifact is MISSING:
  DO NOT proceed to Step 7
  Re-run the missing phase(s) only
  Verify again after re-run
```

**The feature is NOT complete without review-report.md.** Code without
Phase 4 review is incomplete — this is the most commonly skipped phase.
