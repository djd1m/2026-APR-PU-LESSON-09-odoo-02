...
> - FORBIDDEN: implementing features directly without going through /go → /feature pipeline
> - MUST verify all 4 phase artifacts exist after each feature (see Step 2.1)
>
> **KNOWN BUG (2026-05-21):** In autonomous mode the LLM tends to skip Phases 1, 2, 4
> and only do Phase 3 (IMPLEMENT). This is the #1 process compliance failure.
> The verification step below exists specifically to catch and prevent this.
...
while features_remaining_in_scope:
    ...
    /go feature_id                           # complexity router → /plan or /feature
                                             # MUST use Skill tool, NOT raw Agent
    verify_phase_artifacts(feature_id)       # Step 2.1 — MANDATORY
...
### Step 2.1: Phase Artifact Verification (MANDATORY)

After `/go` returns for each feature, check that ALL required phase artifacts
exist. This step exists because autonomous mode has a **proven tendency to skip
Phases 1, 2, and 4** (see insights from 2026-05-21).

```
REQUIRED artifacts for /feature pipeline:
...
REQUIRED artifacts for /plan pipeline:
  ✅ docs/plans/<feature>.md                       — Plan

IF any artifact is MISSING:
  1. DO NOT mark feature as done
  2. Log which phases were skipped
  3. Re-run the MISSING phases (not the whole pipeline)
  4. Only proceed after ALL artifacts verified
```

**CRITICAL:** Do NOT skip this verification. The feature is NOT done until
review-report.md exists. Code without review is incomplete code.
